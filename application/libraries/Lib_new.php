...
class Lib_new {
	...
	function col_count($table,$where){
		if(empty($table)){
			$this->noTable();
		}else{
			if(!empty($where)){
	...
	function jum_milik($where=array()) {
		$this->CI->db->select("*");
		$this->CI->db->from("sarpras");
		$this->CI->db->join("profil","sarpras.npsn=profil.npsn");
		$this->CI->db->where($where);
		$sql = $this->CI->db->get();
		$count = $sql->num_rows();
		return $count;
	...
	function jum_siswa($where=array()) {
		$this->CI->db->select("*");
		$this->CI->db->from("siswa");
		$this->CI->db->join("profil","siswa.npsn=profil.npsn");
		$this->CI->db->where($where);
		$sql = $this->CI->db->get();
		$count = $sql->num_rows();
		return $count;
	...
	function jum_ten($where=array()) {
		$this->CI->db->select("*");
		$this->CI->db->from("tenkependik");
		$this->CI->db->join("profil","tenkependik.npsn=profil.npsn");
		$this->CI->db->where($where);
		$sql = $this->CI->db->get();
		$count = $sql->num_rows();
		return $count;
	...
	function get_sum_col($table,$where=array(),$field){
		if(empty($table)){
			$this->noTable();
		}else{
			if(!empty($where)){
